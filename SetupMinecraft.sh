#!/bin/bash
# Minecraft Server Installation Script - Modified for RHEL / Fedora by Andassaran
#
# NOTE - I CLAIM NO OWNERSHIP OF THIS SCRIPT.  ALL CREDIT GOES TO THE ORIGINAL AUTHOR OF THIS SCRIPT
#
...
# curl https://github.com/Andassarran/MinecraftBedrockServer/raw/refs/heads/master/SetupMinecraft.sh | bash
#
...
# Grab whether this is Fedora, or one of the RHELatives for dependency resolution
...
Update_Scripts() {
  ...
  curl -H "Accept-Encoding: identity" -L -o start.sh https://github.com/Andassarran/MinecraftBedrockServer/raw/refs/heads/master/start.sh
  chmod +x start.sh
  ...
  curl -H "Accept-Encoding: identity" -L -o stop.sh https://github.com/Andassarran/MinecraftBedrockServer/raw/refs/heads/master/stop.sh
  chmod +x stop.sh
  ...
  curl -H "Accept-Encoding: identity" -L -o restart.sh https://github.com/Andassarran/MinecraftBedrockServer/raw/refs/heads/master/restart.sh
  chmod +x restart.sh
  ...
  curl -H "Accept-Encoding: identity" -L -o fixpermissions.sh https://github.com/Andassarran/MinecraftBedrockServer/raw/refs/heads/master/fixpermissions.sh
  ...
  curl -H "Accept-Encoding: identity" -L -o revert.sh https://github.com/Andassarran/MinecraftBedrockServer/raw/refs/heads/master/revert.sh
  chmod +x revert.sh
  ...
  curl -H "Accept-Encoding: identity" -L -o clean.sh https://github.com/Andassarran/MinecraftBedrockServer/raw/refs/heads/master/clean.sh
  chmod +x clean.sh
  ...
  curl -H "Accept-Encoding: identity" -L -o update.sh https://github.com/Andassarran/MinecraftBedrockServer/raw/refs/heads/master/update.sh
  chmod +x update.sh
...
Update_Service() {
  ...
  sudo curl -H "Accept-Encoding: identity" -L -o /etc/systemd/system/$ServerName.service https://github.com/Andassarran/MinecraftBedrockServer/raw/refs/heads/master/minecraftbe.service
...
Check_Dependencies() {
  ...
  if command -v dnf &>/dev/null; then
    ...
    # Fedora does not require the EPEL (Extra Packages for Enterprise Linux) or CodeReady Builder repositories, so skip installation if on Fedora.
    if [[ $OS != "Fedora Linux" ]]; then
      sudo dnf -y install epel-release
      sudo crb enable
    fi
    
    if ! command -v which &>/dev/null; then sudo dnf -y install which; fi
    if ! command -v curl &>/dev/null; then sudo dnf -y install curl libcurl; fi
    if ! command -v unzip &>/dev/null; then sudo dnf -y install unzip; fi
    if ! command -v screen &>/dev/null; then sudo dnf -y install screen; fi
    if ! command -v route &>/dev/null; then sudo dnf -y install net-tools; fi
    if ! command -v gawk &>/dev/null; then sudo dnf -y install gawk; fi
    if ! command -v openssl &>/dev/null; then sudo dnf -y install openssl openssl-libs compat-openssl11; fi
    if ! command -v xargs &>/dev/null; then sudo dnf -y install findutils; fi
    if ! command -v pigz &>/dev/null; then sudo dnf -y install pigz; fi
    ...
    if ! command -v curl &>/dev/null; then sudo dnf -y install curl; fi
    echo "Dependency installation completed"
  else
    echo "Warning: dnf was not found.  You may need to install curl, screen, unzip, libcurl4, openssl, libc6 and libcrypt1 with your package manager for the server to start properly!"
  fi
}
...
Check_Architecture() {
  ...
  if [[ "$CPUArch" == *"aarch"* ]]; then
    ...
    echo "This code path is untested on RHEL/Fedora, use at your own risk."
    
    if [[ $OS != "Fedora Linux" ]]; then
      echo "Due to box64 being unavailable on aarch64 for Red Hat Enterpri1se Linux and derivatives"
      echo "we cannot continue from here.  Please use Fedora Linux version 38 or higher for aarch64."
    exit 1
    fi
    
    sudo dnf -y install box64
    ...
    curl -H "Accept-Encoding: identity" -L -o depends.zip https://github.com/Andassarran/MinecraftBedrockServer/raw/refs/heads/master/depends.zip
    unzip depends.zip
    sudo mkdir /lib64
    ...
    # 32 Bit ARM architecture detected -- download QEMU and dependency libraries
  ...
  if [[ "$CPUArch" == *"i386"* || "$CPUArch" == *"i686"* ]]; then
    ...
    echo "You are running a 32 bit operating system (i386 or i686) and the Bedrock Dedicated Server has only been released for 64 bit (x86_64).  If you have a 64 bit processor please install
 a 64 bit operating system to run the Bedrock dedicated server!"
    exit 1
  fi
}
